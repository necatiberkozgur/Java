...
public class VerticalLine extends Jewel {
	public VerticalLine() {
		mathJewels.add("-");
		mathJewels.add("+");
		mathJewels.add("/");
	...
	public void scanMatchings(ArrayList<ArrayList<String>> grid, int y_pos, int x_pos, User usr) {
		...
		if (x_pos >= 0 && y_pos >= 0 && x_pos <= grid.get(1).size() - 1
			...
			if (x_pos < 2 && y_pos < 2) { /* Upper Left Corner of the Grid */
				if ((grid.get(y_pos + 1).get(x_pos).equals("|") && mathJewels.contains(grid.get(y_pos + 2).get(x_pos)))
						|| (vJewels.contains(grid.get(y_pos + 1).get(x_pos))
								&& vJewels.contains(grid.get(y_pos + 2).get(x_pos)))) {
				...
				if ((grid.get(y_pos - 1).get(x_pos).equals("|") && mathJewels.contains(grid.get(y_pos - 2).get(x_pos)))
					...
					System.out.println("Score: " + point * 3);
					return;
				} else if ((grid.get(y_pos + 1).get(x_pos).equals("|")
